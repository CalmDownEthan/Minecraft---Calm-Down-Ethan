...
class Stealer(object):
    ...
    def elude(self):
        step = 0.2
        gap_y = self.players[0].position[1] - SPAWN_POINT[1]
        if abs(gap_y) >= step:
            factor = 1
            if gap_y > 0:
                factor = -1
        ...
        min_gap = [None, None, None]
        for client_id, player in self.players.items():
            if 0 == client_id or self.players[0].clientid == client_id:
                continue
            if PLAYER_HUNTER == player.player_type:
                for dimension in range(3):
                    if dimension == 1:
                        continue
                    gap = self.players[0].position[dimension] - player.position[dimension]
                    if None == min_gap[dimension] or abs(gap) < abs(min_gap[dimension]):
                        min_gap[dimension] = gap
        ...
        for dimension in range(3):
            if dimension == 1:
                continue
            factor = 1
            gap = min_gap[dimension]
            if abs(gap) > 8:
                continue
            if gap < 0:
                factor = -1
            sp[dimension] += step * factor
            self.players[0].position = self.collide(tuple(sp), PLAYER_HEIGHT)
            #if abs(self.players[0].position[dimension] - sp[dimension]) < step:
            #    sp[dimension] += step * factor * -1 * 2
            #    self.players[0].position = self.collide(tuple(sp), PLAYER_HEIGHT)
    ...
    def on_nick(self, client, client_id, nick):
        ...
        print("stealer joined : [%d,%s]" % (client_id, nick))
...
def main():
    ...
    client = Client(host, int(port), username='stealer')
    stealer = Stealer()
